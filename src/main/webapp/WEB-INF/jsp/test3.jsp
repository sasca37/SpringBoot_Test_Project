...
<div class="container">
    ...
    <form class="form-inline" action="/MagicScheduler/test3" method="get">
        ...
        <div class="form-group">
            ...
            <button type="submit" class="btn btn-primary mb-2 " onclick="func()">조회</button>
        </div>
    </form>
    ...
    <ul class="pagination h-100 justify-content-center align-items-center">
        <c:if test="${paging.prev}">
            <li class="page-item">
                <a class="page-link" onclick="previousBtn()" <%--href='<c:url value="/MagicScheduler/test3?page=${paging.startPage-1}"/>'--%>>이전</a>
            </li>
        </c:if>
            ...
            <c:choose>
                <c:when test="${cri.page eq num}">
                    <li class="page-item active">
                        <a class="page-link" onclick="presentBtn()" <%--href='<c:url value="/MagicScheduler/test3?page=${num}"/>'--%>>${num}</a>
                    </li>
                </c:when>
                <c:otherwise>
                    <li class="page-item">
                        <a class="page-link" onclick="otherBtn()" <%--href='<c:url value="/MagicScheduler/test3?page=${num}"/>'--%>>${num}</a>
                    </li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        <c:if test="${paging.next && paging.endPage>0}">
            <li class="page-item">
                <a class="page-link" onclick="nextBtn()" <%--href='<c:set var="nextBtn" value="${paging.endPage+1}"/>'--%>>다음</a>
            </li>
        </c:if>
    </ul>
...
<script type="text/javascript">
    ...
    function func() {
        ...
        for(let i =0; i<value.length; i++){
            if(value[i] != null && value[i] != '') {
                arr.push(i);
                document.write("ais: " +arr[i]);
                console.log("adsaskdajlksjdakls"+arr[i]);
            }
        }
        if(arr.length ==0 ){
    ...
    function otherBtn() {
        ...
        if(arr.length ==0 ){
            ...
            location.href="test3?page="+page+"&"+key[arr[0]]+"="+value[arr[0]];
        }
        else if (arr.length == 2) {
            location.href="test3?page="+page+"&"+key[arr[0]]+"="+value[arr[0]]+"&"+key[arr[1]]+"="+value[arr[1]];
        }
        else if (arr.length == 3) {
            location.href="test3?page="+page+"&"+key[arr[0]]+"="+value[arr[0]]+"&"+key[arr[1]]+"="+value[arr[1]]+"&"+key[arr[2]]+"="+value[arr[2]];
        }
        else if (arr.length == 4) {
            location.href="test3?page="+page+"&"+key[arr[0]]+"="+value[arr[0]]+"&"+key[arr[1]]+"="+value[arr[1]]+"&"+key[arr[2]]+"="+value[arr[2]]+key[arr[3]]+"="+value[arr[3]];
        }
        else {
            location.href= "test3?contract_date="+contract_date+"&registration_num="+registration_num+"&status="+status+
                "&created_start="+created_start+"&created_end="+created_end;
    ...
    function nextBtn(){
        var page = '<c:out value="${paging.endPage+1}"/>';
        var contract_date = document.getElementById("contract_date").value;
        var status = document.getElementById("status").value;
        var registration_num = document.getElementById("registration_num").value;
        var created_start = document.getElementById("created_start").value;
